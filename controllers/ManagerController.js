...
exports.managersignup = (req, res) => {
    ...
    Manager.findOne({ email: email })
        .then(user => {
            ...
            // Save the manager data directly (no verification required)
            manager.save()
                .then(() => {
                    return res.status(200).json({ success: true, message: "Account created successfully" });
                })
                .catch((err) => {
                    return res.status(500).json({ error: "Something went wrong, please try again later", success: false });
                });
        })
        .catch((err) => {
...
exports.managerlogin = async (req, res) => {
    ...
    try {
        // Find the manager by email and password
        ...
        // If token is generated, set the token in a cookie and return response
        if (token) {
            res.cookie('jwtoken', token, {
                expires: new Date(Date.now() + 2589200000), // Token expiry (e.g., 30 days)
                httpOnly: true
            });
            ...
            return res.status(200).json({
                ...
                token,
                user: { fname },
                type: "Manager"  // Change from "Influencer" to "Manager"
            });
        } else {
